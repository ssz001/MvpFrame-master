...
import com.ssz.frame.utils.Constant;
import com.ssz.frame.utils.IOUtils;
import com.ssz.frame.utils.RxUtils;
...
public abstract class PageLoader {
    ...
    private void initPaint() {
        ...
        mTipPaint.setTextSize(ScreenUtil.spToPx(DEFAULT_TIP_SIZE)); // Tip默认的字体大小
...
//           if(isInit)
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        int tipMarginHeight = ScreenUtil.dpToPx(3);
        if (!isUpdate) {
            /****绘制背景****/
            ...
            //擦除区域
            mBgPaint.setColor(mBgColor);
            canvas.drawRect(mDisplayWidth / 2, mDisplayHeight - mMarginHeight + ScreenUtil.dpToPx(2), mDisplayWidth, mDisplayHeight, mBgPaint);
        }
        ...
        int polarHeight = ScreenUtil.dpToPx(6);
        int polarWidth = ScreenUtil.dpToPx(2);
        int border = 1;
        int innerMargin = 1;
        ...
        int outFrameBottom = visibleBottom - ScreenUtil.dpToPx(2);
        ...
        float x = outFrameLeft - mTipPaint.measureText(time) - ScreenUtil.dpToPx(4);
